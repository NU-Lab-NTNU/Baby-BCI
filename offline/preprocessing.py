...
def artifact_rejection(
    ...
    normalised_envelope_sum = np.absolute(
        np.sum(normalised_envelope, axis=0) / np.sqrt(normalised_envelope.shape[0])
    )[croplen:-croplen]
    ...
    low_voltage_window1 = np.logical_and(
        samples_below_limit[:, :-3], samples_below_limit[:, 1:-2]
    )
    low_voltage_window2 = np.logical_and(
        samples_below_limit[:, 2:-1], samples_below_limit[:, 3:]
    )
...
def notch_filter(eeg_data, sampling_freq, centre_freq, sharpness):
    numerator_coeffs, denominator_coeffs = signal.iirnotch(
        centre_freq, sharpness, sampling_freq
    )
...
def bandpass_filter(
    eeg_data, sampling_freq, freq_low_limit, freq_high_limit, filter_order
):
    sos = signal.butter(
        filter_order,
...
def plot_channels(
    ...
    for channel_idx, channel_id in enumerate(channels_to_plot):
        if color is not None:
            plt.plot(
                time_in_millis,
                eeg_data[channel_id] - channel_idx * voffset,
                label=f"{ch_prefix}{channel_id+1}",
                color=color,
            )
        else:
            plt.plot(
...
def preprocess(
    ...
    padded_eeg_data = np.zeros(
        (preprocessed_eeg_data.shape[0], preprocessed_eeg_data.shape[1] + 2 * padlen)
    )
    padded_eeg_data[:, padlen:-padlen] = preprocessed_eeg_data
    padded_eeg_data = notch_filter(
        padded_eeg_data, sampling_freq, notch_centre_freq, notch_sharpness
    )
    padded_eeg_data = bandpass_filter(
        padded_eeg_data,
        sampling_freq,
        bandpass_low,
        bandpass_high,
    ...
    preprocessed_eeg_data, ar_code, bad_ch, v_high = artifact_rejection(
        preprocessed_eeg_data, z_t, v_t_h, v_t_l
    )
...
def preprocess_dataset(age, source_dir, target_dir, speed_key):
    ...
    minimum_voltage = 0.01
    raw_eeg_data, peak_mask, file_ids, peak_samples, speed, _ = load_xyidst_threaded(
    ...
    preprocessed_eeg_data, good_trials, bad_chs, ar_codes = preprocessing(
        raw_eeg_data, maximum_voltage=maximum_voltage, minimum_voltage=minimum_voltage
    )
    num_removed = int(good_trials.shape[0] - np.sum(good_trials))
    print(
        f"Finished preprocessing, removed {num_removed} trials during artifact rejection"
    )
    ...
    preprocessed_eeg_data, peak_mask, file_ids, peak_samples, speed, bad_chs = resample(
        preprocessed_eeg_data, peak_mask, file_ids, peak_samples, speed, bad_chs
    )
    num_trials_with_peaks = np.sum(peak_mask == 1)
    num_trials_without_peaks = np.sum(peak_mask == 0)
    print(
    ...
    first_trial_raw_eeg = (
        first_trial_raw_eeg.T - np.mean(first_trial_raw_eeg, axis=1)
    ).T
...
def combine_datasets():
    for phase in ["train/", "val/", "test/"]:
        ...
        xl, yl, idsl, erp_tl, speedl, bad_chl = load_xyidst_threaded(
            folder, load_bad_ch=True, verbose=True
        )

        folder = "data/greaterthan7/dataset/preprocessed/" + phase
        xg, yg, idsg, erp_tg, speedg, bad_chg = load_xyidst_threaded(
            folder, load_bad_ch=True, verbose=True
        )
    ...
    for age, source_dir, target_dir in zip(AGES, source_folders, target_folders):
        ...
        for speed_key in SPEED_KEYS:
            p = multiprocessing.Process(
                target=preprocess_dataset, args=(age, source_dir, target_dir, speed_key)
            )
            p.start()
            processes.append(p)
