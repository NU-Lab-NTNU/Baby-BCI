...
FAST_TRIAL_DURATION = 2
MEDIUM_TRIAL_DURATION = 3
SLOW_TRIAL_DURATION = 4
...
PRE_COLL_DURATION = {
    "fast": FAST_TRIAL_DURATION,
    "medium": MEDIUM_TRIAL_DURATION,
    "slow": SLOW_TRIAL_DURATION,
}

AGES = ["less", "greater"]
SPEED_KEYS = ["fast", "medium", "slow"]
DATA_FOLDER = "data_prec_it/"
...
def read_raw_file(fname, speed_key):
    triggers = ["stm+"]
    event = "stm-"
    try:
    ...
    coll_ts = (sample_num[coll_events == 1]).astype(int) - int(PRE_COLL_DURATION[speed_key] * sfreq)
    start_ts = (coll_ts - int(INTER_TRIAL_DURATION * sfreq)).astype(int)

    return True, egi, start_ts, coll_ts, sfreq, 0


def read_raw_data(egi, start_ts, coll_ts):
    N_ch = 128
    data_ch_idx = [i for i in range(N_ch)]
    N_samples = coll_ts[0] - start_ts[0]
    N_trials = coll_ts.shape[0]
...
def remove_wrong_speed(start_ts, coll_ts, erp, erp_ts, speed, speed_key):
    correct_speed_mask = speed == KEY_SPEED_DICT[speed_key]
    start_ts = start_ts[correct_speed_mask]
    coll_ts = coll_ts[correct_speed_mask]
    erp = erp[correct_speed_mask]
    erp_ts = erp_ts[correct_speed_mask]
    speed = speed[correct_speed_mask]
    return start_ts, coll_ts, erp, erp_ts, speed


def extract_trials(fname, speed_key):
    raw_success, egi, raw_start_ts, raw_coll_ts, sfreq, error_code = read_raw_file(
        fname,
        speed_key
    )
    ...
    if raw_success:
        ...
        if evt_success:
            if raw_coll_ts.shape == erp.shape:
                raw_start_ts, raw_coll_ts, erp, erp_ts, speed = remove_wrong_speed(raw_start_ts, raw_coll_ts, erp, erp_ts, speed, speed_key)
                data_success, eeg, error_code = read_raw_data(
                    egi, raw_start_ts, raw_coll_ts
                )
                if data_success:
